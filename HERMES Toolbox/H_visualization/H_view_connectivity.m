function varargout = H_view_connectivity ( varargin )
...
% H_VIEW_CONNECTIVITY M-file for H_view_connectivity.fig
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @H_view_connectivity_OpeningFcn, ...
                   'gui_OutputFcn',  @H_view_connectivity_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function H_view_connectivity_OpeningFcn ( hObject, eventdata, handles, varargin )
...
% Disables the p-value panel by default.
set ( handles.pvalue, 'String', '1' )
...
% Enables the p-value filter if surrogates where calculated.
if handles.data.index.config.statistics == 1
    set ( handles.pvalue, 'Enable', 'On' )
    set ( handles.pvalue, 'String', '0.05' );
    handles.PVAL = str2double ( get ( handles.pvalue, 'String' ) );
end
...
% Checks that the subject selected exists.
subject  = str2double ( get ( handles.subject, 'String' ) );
subjects = numel ( handles.data.project.subjects );

if H_checkOR ( subject, 'nan', '~real', 'inf', '~int' )
    warndlg ( 'Wrong subject''s number', 'Subject''s number warning' );
    set ( handles.subject, 'String', 1 )
end

% Corrects the values out of range and enables or disables the buttons.
if subject < 1,            set ( handles.subject, 'String', 1 )
elseif subject > subjects, set ( handles.subject, 'String', subjects )
end

if subject == 1,        set ( handles.prev, 'Enable', 'Off' )
else                    set ( handles.prev, 'Enable', 'On' )
end
if subject == subjects, set ( handles.next, 'Enable', 'Off' )
else                    set ( handles.next, 'Enable', 'On' )
end

% Checks that the p-value is valid.
if handles.data.index.config.statistics == 1
    pval = str2double ( get ( handles.pvalue, 'String' ) );
    
    if H_checkOR ( pval, 'nan', '~real', 'inf', 'lt0', 'gt1' )
        warndlg ( 'Wrong p-value', 'p-value warning' );
        set ( handles.pvalue, 'String', 0.05 )
    end
end
...
% Gets the subject and condition.
subject   = str2double ( get ( handles.subject, 'String' ) );
condition = get ( handles.condition, 'Value' );

% Gets the data.
data = handles.data.index.data { condition, subject };

% Applies the permutation mask to the current subject and condition.
if handles.data.index.config.statistics == 1
    pval = str2double ( get ( handles.pvalue, 'String' ) );
    data ( handles.data.index.pval { condition, subject } > pval ) = 0;
end

% Corrects the value in H, M and N indexes.
% index = handles.data.indexes { get ( handles.INDEX_popupmenu, 'Value' ) };
index = handles.data.indexes { get ( handles.index, 'Value' ) };
...
function subject_Callback      ( hObject, eventdata, handles ), show ( handles )
...
function pvalue_Callback       ( hObject, eventdata, handles ), show ( handles )
...
function H_view_connectivity_CloseRequestFcn ( hObject, eventdata, handles ), uiresume
function H_view_connectivity_OutputFcn       ( hObject, eventdata, handles ), delete ( hObject )
